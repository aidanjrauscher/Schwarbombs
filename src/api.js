...
export async function fetchAllStats(season) {
  ...
  const [playerData, teamData, careerData, scheduleData, leaderData] = await Promise.all([
    ...
    fetch(`https://statsapi.mlb.com/api/v1/stats/leaders?leaderCategories=homeRuns&season=${season}&sportId=1&limit=50`).then(r => r.json()),
  ])
  ...
  const leaderEntries = leaderData?.leagueLeaders?.[0]?.leaders ?? []
  const hrRank = leaderEntries.find(e => e.person?.id === PLAYER_ID)?.rank ?? null
  ...
  if (games.length === 0) {
    return { stats, teamGamesPlayed, careerHR, hrRank, todayHR: null, noGameToday: true }
  }
  ...
  return { stats, teamGamesPlayed, careerHR, hrRank, todayHR, noGameToday: false }
}
